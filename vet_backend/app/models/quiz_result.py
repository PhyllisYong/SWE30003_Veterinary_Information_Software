...
class QuizResult(Base):
    ...
    totalScore = Column("total_score", Integer, nullable=False, default=0)
    ...
    def getPassed(self) -> bool:
        if self.quiz and self.quiz.totalScore:
            return self.totalScore >= (self.quiz.totalScore * 0.6)
        return False

    def getSummary(self) -> str:
        passed = "Passed" if self.getPassed() else "Failed"
        return f"Score: {self.totalScore} | {passed} | Attempted: {self.attemptedAt}"
